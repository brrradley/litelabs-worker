...
RUN python /app/litelabs_parent_preset_analysis_patch.py \
    && python -m py_compile /app/preset_pack.py /app/litelabs_parent_preset_analysis_patch.py \
    && python - <<'PY'
...
print('LiteLABS production overlay verified: Basic/Core validated analysis enabled')
PY
