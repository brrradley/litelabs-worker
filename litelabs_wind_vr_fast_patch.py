...
new = '["audio-separator", str(other_parent), "--model_filename", WIND_MODEL, "--model_file_dir", str(audio_separator_model_dir), "--output_dir", str(wind_out), "--output_format", "FLAC", "--vr_batch_size", "16", "--vr_window_size", "512"]'
if old not in text:
...
replacement = needle + '                "inference_profile": {"vr_batch_size": 16, "vr_window_size": 512},\n'
if needle not in text:
    raise RuntimeError('Could not locate wind report architecture')
text = text.replace(needle, replacement, 1)

path.write_text(text, encoding='utf-8')
print('LiteLABS fast VR wind profile applied: batch=16 window=512')
