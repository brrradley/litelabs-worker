...
new = '["audio-separator", str(other_parent), "--model_filename", WIND_MODEL, "--model_file_dir", str(audio_separator_model_dir), "--output_dir", str(wind_out), "--output_format", "FLAC", "--vr_batch_size", "32", "--vr_window_size", "1024", "--use_autocast"]'
if old not in text:
...
replacement = needle + '                "inference_profile": {"vr_batch_size": 32, "vr_window_size": 1024, "autocast": True, "quality_note": "speed-oriented VR profile; compare against 512-window reference"},\n'
if needle not in text:
    raise RuntimeError('Could not locate wind report architecture')
text = text.replace(needle, replacement, 1)

path.write_text(text, encoding='utf-8')
print('LiteLABS aggressive fast VR wind profile applied: batch=32 window=1024 autocast')
