...
cap_anchor = '    audio_url = payload.get("audio_url")\n'
cap_insert = '''    if payload.get("capabilities") is True:\n        from preset_pack import preset_capabilities\n        return preset_capabilities()\n\n'''
if cap_anchor not in text:
    raise RuntimeError('Could not locate audio_url anchor for capabilities endpoint')
text = text.replace(cap_anchor, cap_insert + cap_anchor, 1)
...
print('LiteLABS BASIC/CORE/EXPERIMENTAL preset dispatcher and capabilities applied')
