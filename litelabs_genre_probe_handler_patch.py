...
if marker not in text:
    raise RuntimeError('Could not locate handler payload anchor for genre probe')

if 'genre_probe_handler_v1' not in text:
    block = '''    # genre_probe_handler_v1
    if str(payload.get("mode") or "").strip() == "genre_probe":
        ...
        if not audio_url and not local_audio_path:
            return {
                "ok": False,
                "mode": "genre_probe",
                "error": "genre_probe requires input.audio_url or input.audio_path",
            }

        try:
            timeout_seconds = max(
                30,
                int(payload.get("genre_timeout_seconds") or 300),
            )
            with _tempfile.TemporaryDirectory(
                prefix="litelabs_genre_probe_"
            ) as temp_dir:
                if local_audio_path:
                ...
                if not output_lines:
                    raise RuntimeError(
                        "G400 genre probe returned no JSON output"
                    )
...
assert 'genre_probe_handler_v1' in check
assert '"/app/genre_probe.py"' in check
print('LiteLABS isolated genre probe handler route applied')
